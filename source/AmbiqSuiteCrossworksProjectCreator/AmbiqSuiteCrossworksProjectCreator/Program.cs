...
namespace AmbiqSuiteCrossworksProjectCreator
{
	class Program
	{
		...
		{
			Console.WriteLine("Please copy this tool into the root folder of the AmbiqSuite and execute");
			...
			//
			// Create crossworks sub-directory if not existing
			//
			if (!Directory.Exists(Path.Combine(TargetFolder,"crossworks")))
			{
				Directory.CreateDirectory(Path.Combine(TargetFolder,"crossworks"));
			}
			...
			if (dInfoTarget.Name.Equals("bsp") || dInfoTarget.Name.Equals("hal"))
			{
				...
				//
				CrossworksProject emPrj = new CrossworksProject("libam_" + dInfoTarget.Name,Path.Combine(TargetFolder,"crossworks"),mParser,lParser,true);
			    emPrj.Create();
			} else 
			{
				//
				...
				//
				CrossworksProject emPrj = new CrossworksProject(dInfoTarget.Name,Path.Combine(TargetFolder,"crossworks"),mParser,lParser,false);
			    emPrj.Create();
			    
			    //
			    // A normal project needs also a Crossworks-IDE compatible startup - read-in GCC startup and converting to Crossworks style
			    //
			    ...
			    if (fIfos.Length > 0)
			    {
				    ...
				    sParser.Create(Path.Combine(Path.Combine(TargetFolder, "crossworks"),emPrj.StartupFileName));
			    } else
			    {
