...
{
	/// <summary>
	/// Description of SeggerEmProject.
	/// </summary>
	public class SeggerEmProject
	{
		...
		public void Create()
		{
			...
			foreach(KeyValuePair<string,FileInfo> kp in makeFile.LibraryFiles)
			{
				...
				DirectoryInfo dInfoHalSegger = new DirectoryInfo(Path.Combine(dInfoHal.FullName,"segger"));
				FileInfo dSeggerProject = new FileInfo(Path.Combine(dInfoHalSegger.FullName,Name + ".emProject"));
				
                //
				// If a Segger project exists within the library path, 
				// use the project as dependency instead of the library
				//
				// Otherwise use the GCC library
				//
				if (dSeggerProject.Exists) 
				{
					Uri uriRoot = new Uri(ProjectFolder);
					Uri uriFile = new Uri(dSeggerProject.FullName);
			...
			StreamWriter sWriter = new StreamWriter(Path.Combine(ProjectFolder,ProjectName + ".emProject"));
			...
			//
			File.WriteAllText(Path.Combine(ProjectFolder,"flash_placement.xml"),global::AmbiqSuiteSeggerProjectCreator.Templates.flash_placement_xml);
			File.WriteAllText(Path.Combine(ProjectFolder,"ram_placement.xml"),global::AmbiqSuiteSeggerProjectCreator.Templates.ram_placement_xml);
		}
	}
}
